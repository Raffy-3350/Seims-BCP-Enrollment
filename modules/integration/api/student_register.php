...
// FIX: replaced MySQL YEAR() with PostgreSQL EXTRACT(YEAR FROM ...)
$year  = date('Y');
$stmt  = $conn->prepare("SELECT COUNT(*) AS cnt FROM students WHERE EXTRACT(YEAR FROM created_at) = ?");
$stmt->execute([$year]);
$row   = $stmt->fetch();
$count = (int) $row['cnt'];
...
// FIX: uses PDO execute([]) instead of mysqli bind_param
$stmt = $conn->prepare("
...
$stmt->execute([
    $regId, $fn, $mn, $ln, $bd, $gen,
    $mob, $email,
    $addr, $city, $prov, $zip,
    $prog, $yr, $major, $estatus,
    $gname, $grel, $gcont
]);
...
echo json_encode([
    ...
    'message'        => 'Registration submitted. Status: pending.'
]);
